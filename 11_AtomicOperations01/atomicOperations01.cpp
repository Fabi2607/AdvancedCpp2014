...
void demonstrate_race_condition( void ) {
    ...
    BOOST_LOG_TRIVIAL( debug ) << "initial value of n: " << n;
    std::mutex m;
    std::thread t3( slow_protected_increment, std::ref( n ), true,
                    std::ref( m ) );
    std::thread t4( slow_protected_increment, std::ref( n ), false,
                    std::ref( m ) );
    t3.join();
    t4.join();
    BOOST_LOG_TRIVIAL( debug ) << "value of n: " << n;
...
void atomic_types_lock_free( void ) {
    ...
    std::atomic_ulong aULong(
        (unsigned long)aLong );             // std::atomic<unsigned long>
