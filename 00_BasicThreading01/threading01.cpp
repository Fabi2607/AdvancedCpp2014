 ...
 * needed for dynamyic linked boost log libraries
 */
 ...
 */
void foo( void ) {
    BOOST_LOG_TRIVIAL( debug ) << "foo called in thread: "
...
void start_simple_thread( void ) {
    BOOST_LOG_TRIVIAL( debug ) << ">>>void start_simple_thread(void)";
...
void start_thread_array( void ) {
    BOOST_LOG_TRIVIAL( debug ) << ">>>void start_thread_array(void)";

    // declaring an array of threads
    int max_threads = thread::hardware_concurrency() - 1;
    thread t[max_threads]; // "Not-a-thread" threads until constructor is
                           // called
...
void start_detached_thread( void ) {
    BOOST_LOG_TRIVIAL( debug ) << ">>>void start_detached_thread(void)";
...
void move_swap_thread( void ) {
    ...
    std::thread t4{
        std::move( t3 )}; // constructor call with move => no copies allowed
...
int main( int argc, char **argv ) {
    ...
    BOOST_LOG_TRIVIAL( debug ) << "Number of Hardware Threads: "
                               << std::thread::hardware_concurrency();

    // get thread id
    BOOST_LOG_TRIVIAL( debug ) << "ID of main - thread: "
                               << std::this_thread::get_id();
