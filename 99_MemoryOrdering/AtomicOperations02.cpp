...
void atomic_memory_order_increment( std::atomic_int &n, bool mode,
...
void memory_order_options( void ) {
    // memory order allows for thread synchronization in addition to race
    ...
    */
    ...
    t1 = std::thread( atomic_memory_order_increment, std::ref( n ), true,
                      std::memory_order_relaxed );
    t2 = std::thread( atomic_memory_order_increment, std::ref( n ), false,
                      std::memory_order_relaxed );
    t1.join();
    t2.join();
